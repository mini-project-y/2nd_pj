...
year = sys.argv[1]
address_df = pd.read_csv(f'Data_Preprocessing/add_{year}.csv', encoding="utf-8")
...
for row in address_df.values:
    ...
    time.sleep(2)  # 0.2초 대기 (초당 5건 요청)

# 결과를 데이터프레임으로 변환
coordinates_df = pd.DataFrame(coordinates)

# 결과 저장
coordinates_df.to_csv(f"geocoded_addresses_{year}.csv", index=False)

...
file_path = os.path.join(base_dir, "Data_Preprocessing", "hospital.csv")
...
for idx, row in address_df.iterrows():
    no = row[10]  # 첫 번째 열 (학교 번호 또는 이름)
    address = row[1]  # 세 번째 열 (도로명 주소)
...
# 결과 저장
output_path = os.path.join(base_dir, "geocoded_hospital.csv")
